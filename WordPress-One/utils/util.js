...
// 绘制海报文字：文章题目、摘要、扫码阅读
function drawTitleExcerpt(context,content) {
    context.setFillStyle("#000000");
    context.setFontSize(24);
    context.setTextAlign('left');
    context.setGlobalAlpha(0.7);
    var contents = removeHTML(content);
    for (var i = 0; i <= 80; i += 18) {
        //摘要只绘制前50个字，这里是用截取字符串
        if (getStrLength(contents)>80) {
            if ( i == 40) {
              context.fillText(contents.substring(i, i + 18) + "...", 80, 540 + i * 2);
            } else {
              context.fillText(contents.substring(i, i + 18), 80, 540 + i * 2);
            }
        } else {
